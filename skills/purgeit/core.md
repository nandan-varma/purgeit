...
on their behalf) explicitly selects. Safety is the entire point of the tool: local matching is
...
without an explicit selection step — there is no "clean everything" mode. purgeit also finds
forgotten cloud dev environments (AWS/GCP) — see `npx purgeit skills get cloud` for that surface.
...
- Run `npx purgeit skills get core --full` for the complete list of library exports.
